...
LazySeq<std::pair<K, V>> trace(const LazySeq<std::pair<K, V>> &seq) {
  seq.template foreach<std::pair<K, V>>(tracePair<K, V>);
  std::cout << std::endl;
  return seq;
}
...
void testSmartSkips() {
  ...
  for (auto seq: {naturalNumbers().groupBy(5), naturalNumbers().groupBy(5).skip(2),
...
void testFunctionsByIndex() {
  ...
  trace(range1to5.minByIndex<wide_size_t>(multiplicationCriterion, std::greater<>()));
  trace(range1to5.maxByIndex(multiplicationCriterion));
  trace(range1to5.maxByIndex<wide_size_t>(multiplicationCriterion, std::greater<>()));
  ...
  trace(values(seq.orderByIndexBy(multiplicationCriterion)));
  trace();
  writePairs(seq.orderByDescendingByIndexBy(multiplicationCriterion));
  trace(values(seq.orderByDescendingByIndexBy(multiplicationCriterion)));
  trace();
}

void testForeach() {
  trace("testForeach");
  range<size_t>(1, 10).foreach<size_t>(trace<size_t>);
  range<size_t>(1, 10).map<size_t>(trace<size_t>).foreach();
...
void testOrder() {
  ...
  trace(seq4.orderBy<char>(charGetter(0))
            .thenByDescending<char>(charGetter(1))
            .thenByDescending<char>(charGetter(2)));
  ...
  auto seq5 = seq4.orderBy<char>(charGetter(0)).thenBy<char>(charGetter(1));
  trace(seq5);
...
void testGroupBy() {
  ...
  for (const auto &seq: {range(1, 10).groupBy<int, std::string>(keyFinder, seqFunc),
                         range(1, 10).groupBy<int, int, std::string>(keyFinder, valueFunc, seqFunc)}) {
    trace(seq);
  }
...
void testCartesianMultiplication() {
  ...
  range(1, 10).pow<3>().take(200).foreach<void>([](const auto &tup) {
    std::cout << "("
              << std::get<0>(tup) << ", "
              << std::get<1>(tup) << ", "
              << std::get<2>(tup) << ")\n";
...
void testArithmeticOperations() {
  ...
  trace(range(1, 4).sum());
  const auto func = partial(std::multiplies<int>(), 2);
  trace(range(1, 4).sum<int>(func));
  trace(range(1, 4).average<float>());
  trace(range(1, 4).average<int, float>(func));
  trace(range(1, 4).subtract());
  trace(range(1, 4).subtract<int>(func));
  trace(range(1, 4).multiply());
  trace(range(1, 4).multiply<int>(func));
  trace(range(1, 4).castTo<float>().divide());
  trace(range(1, 4).castTo<float>().divide<float>(func));
}
...
void testMatch() {
  ...
  trace(range('a', 8).match(range<size_t>(1, 100)).toString<std::string>(func));
  trace(range('a', 8).match<size_t, std::string>(range<size_t>(1, 100), func));
}
