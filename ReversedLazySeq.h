...
public:
    ...
    constexpr ReversedLazySeq(const LazySeq<T> &reversedSeq, LazySeq<T> nonReversedSeq);
    ...
    [[nodiscard]] constexpr ReversedLazySeq<T> rest() const;

    [[nodiscard]] constexpr ReversedLazySeq<T> butLast() const;

    template<class Func, class R = ResType<Func, T>>
    constexpr ReversedLazySeq<R> map(const Func &func) const;

    [[nodiscard]] constexpr ReversedLazySeq<T> concat(const LazySeq<T> &other) const;
